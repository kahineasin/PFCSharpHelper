...
//                    ent = new ZipEntry(Path.Combine(parentFolderName, Path.GetFileName(folderToZip) + "/" + Path.GetFileName(file)));//原版,生成的压缩文件内部路径为绝对路径 _D\\xx\\xx 
//                    //ent = new ZipEntry(Path.Combine(Path.GetFileName(parentFolderName), Path.GetFileName(folderToZip) + "/" + Path.GetFileName(file)));//benjamin
