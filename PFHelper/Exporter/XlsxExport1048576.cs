...
namespace Perfect
{
    ...
    /// </summary>
    public class XlsxExport1048576 : IExport
    {
        ...
        public virtual void FillData(int x, int y, string field, object value)
        {
            ...
            switch ((value ?? string.Empty).GetType().Name.ToLower())
            {
                ...
                case "int64":
                case "decimal":
                    tmpSheet.Cells[tmpY, x].PutValue(PFDataHelper.ObjectToType<double>(value, 0));
                    break;
                //case "System.String[]":
                //    var s = String.Join(",", (string[])value);
                //    sheet.Cells[y, x].PutValue(s);
                ...
                default:
                    ...
                    {
                        var s = String.Join(",", value as string[]);
                        tmpSheet.Cells[tmpY, x].PutValue(s);
                    }
                    else
                    {
                        tmpSheet.Cells[tmpY, x].PutValue(PFDataHelper.ObjectToString(value));
                    }
                    break;
            }
        }
        ...
        public virtual void SetHeadStyle(int x1, int y1, int x2, int y2)
        {
            ...
            for (var y = y1; y <= y2; y++)
            {
                ...
                if (_printPageScheme != null) { tmpSheet.Cells.SetRowHeight(tmpY, _printPageScheme.HeadRowHeight); }
            }
        }
        ...
        public virtual void SetRowsStyle(int x1, int y1, int x2, int y2)
        {
            ...
            for (var y = y1; y <= y2; y++)
            {
                ...
                if (_printPageScheme != null) { tmpSheet.Cells.SetRowHeight(tmpY, _printPageScheme.DataRowHeight); }
            }
        }
